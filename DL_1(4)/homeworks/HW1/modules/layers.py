...
class Linear(Module):
    ...
    # Все формулы для расчёта градиентов были взяты из https://robotchinwag.com/posts/linear-layer-deriving-the-gradient-for-the-backward-pass/
...
class BatchNormalization(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        if self.training:
            B = input.shape[0]
            self.mean = np.sum(input, axis=0) / B
            self.input_mean = input - self.mean
            self.var = np.sum((input - self.mean)**2, axis=0) / B
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        if self.training == True:
            dvar = np.sum(grad_norm * self.input_mean * (-0.5) * (self.inv_sqrt_var**3), axis=0)
            dmean = np.sum(grad_norm * (-self.inv_sqrt_var), axis=0) + dvar * np.sum(-2 * self.input_mean, axis=0)/self.input_mean.shape[0]
            grad_input = (grad_norm * self.inv_sqrt_var + 
                        dvar * 2 * self.input_mean / B + 
                        dmean / B)
        else:
            grad_input = grad_norm * self.inv_sqrt_var
        return grad_input
...
class Dropout(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        if self.training == True:
            ...
            output =  self.mask / (1 - self.p) * input
        else:
            output = input
        return output
